...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8081/components/home');
  ...
  return (
    <div className='mx-auto p-8 max-w-screen-md mt-20'>
      ...
      <div className='flex justify-center items-center'>
        <Link to="/front" className="bg-green-500 hover:bg-green-800 text-white font-bold py-2 px-4 rounded">Home</Link>
      </div>
